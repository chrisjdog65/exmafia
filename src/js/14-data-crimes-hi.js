/* ============================================================
   ...
   THE PAY AND XP FIGURES BELOW ARE SEEDS, NOT FINAL NUMBERS.
   45-normalize fitJobPay() rescales every job in the game at boot
   against whatever the crime table actually says, so retuning a
   crime drags the jobs along behind it and no hand-tuned number
   here can drift out of line. What survives from the literals in
   this file is the SHAPE - the spread between pay[0] and pay[1],
   and each job's size relative to its neighbours. What decides
   the level is fitJobPay's own target:

     share(L) = 0.40 - 0.10 * min(1, L / 140), floored at 0.28

   of a day of same-level crime, with the Energy cost, the Labour
   multiplier and the crime's own failure rate all divided out
   first. Measured on the built file, a shift lands at 22-27% of
   a day of crime across all ten of these and 20-26% per action -
   the "roughly a fifth" the original game ran at - and the join
   is invisible: 25% at Consigliere (74, old) into 24% at Bonding
   Agent (80, new).

   THE ENERGY COSTS ARE REAL, AND THEY ARE THE LEVER. fitJobPay
   divides by j.energy, so these ten numbers set how much of the
   day a shift eats and therefore what the pay is solved to. The
   old table had already reached 22 Energy by level 74, so this
   ladder starts at 14 and climbs to 34 rather than restarting at
   the bottom; that is why it jumps where it does instead of
   climbing one a rung.

   pay/Energy and xp/Energy both rise strictly with level across
   every job from level 10 up - fitJobPay runs a final pass that
   guarantees it - including where these ten interleave with the
   old ones at 55, 66 and 71. Nothing here dominates a job below
   it, and nothing here out-earns the crime you could be pulling
   instead.
